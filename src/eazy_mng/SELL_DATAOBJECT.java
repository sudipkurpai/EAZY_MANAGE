...
public class SELL_DATAOBJECT {
        ...
        int status=0;
              try{
                   ...
                   PreparedStatement ps=con.prepareStatement("INSERT INTO sell (Employee_name,Emp_id,Date,Time,"
                           + "Product_name,Product_id,Quantity,Price,Total) VALUES (?,?,?,?,?,?,?,?,?)");
 ...
 public static int bill (String Product_id, String Quantity,String Master )
           {
        int status=0;
              try{
                   Connection con=DATABASE_CONNECTION.getConnection();  
                   PreparedStatement ps=con.prepareStatement("INSERT INTO bill_time (Product_id,Quantity,Master) VALUES (?,?,?)");
